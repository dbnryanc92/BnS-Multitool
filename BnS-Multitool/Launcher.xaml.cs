        ...
        private async Task LaunchNewGameClient()
        {
            try
            {
                ...
                else if (!ACCOUNT_CONFIG.ACCOUNTS.Saved[ACCOUNT_SELECTED_INDEX].PINCODE.IsNullOrEmpty())
                {
                    ...
                    proc.StartInfo.EnvironmentVariables["BNS_PROFILE_PIN"] = otp;
                    // dbnryanc92 version: Keep OTP calculation in multitool to avoid stuck at otp input prompt (loginhelper bug)
